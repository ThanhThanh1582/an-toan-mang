...
function executeMove(cardId, isDroppedOnSafeLane) {
  ...
  const cardData = sortingCards.find(c => c.id === cardId);
  if (!cardEl || !cardData) return;
  
  const correctType = cardData.type;
  const isCorrect = (isDroppedOnSafeLane && correctType === 'safe') || (!isDroppedOnSafeLane && correctType === 'danger');
  ...
  if (isCorrect) {
    sound.playSuccess();
    cardEl.style.borderColor = isDroppedOnSafeLane ? "#10B981" : "#EF4444";
    cardEl.style.boxShadow = isDroppedOnSafeLane ? "0 0 10px rgba(16,185,129,0.2)" : "0 0 10px rgba(239,68,68,0.2)";
    ...
    document.getElementById('sorting-game-bank').appendChild(cardEl);
    setTimeout(() => {
