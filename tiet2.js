...
function initSortingGame() {
  ...
  // Make bank a dropzone too to drag cards back
  bank.ondragover = (e) => {
    e.preventDefault();
    bank.style.background = "rgba(15, 23, 42, 0.04)";
  };
  bank.ondragleave = () => {
  ...
  bank.ondrop = (e) => {
    ...
    if (cardEl) {
      cardEl.style.borderColor = "rgba(15,23,42,0.1)";
  ...
  const dropzones = [
    { el: safeDz, isSafe: true },
    { el: dangerDz, isSafe: false }
  ];
  
  dropzones.forEach(dzObj => {
    const dz = dzObj.el;
    
    dz.ondragover = (e) => {
      e.preventDefault();
      dz.style.background = dzObj.isSafe ? "rgba(16, 185, 129, 0.08)" : "rgba(239, 68, 68, 0.08)";
    };
    ...
    dz.ondrop = (e) => {
      ...
      dz.style.background = "";
      const cardId = e.dataTransfer.getData("text/plain");
      executeMove(cardId, dzObj.isSafe);
    };
    
    dz.onclick = () => {
      if (selectedCard) {
        executeMove(selectedCard.id, dzObj.isSafe);
        selectedCard.classList.remove('selected-card');
        selectedCard.style.border = "1px solid rgba(15,23,42,0.1)";
        selectedCard = null;
      }
    };
  });
  ...
  shuffled.forEach(c => {
    ...
    cardEl.style.cssText = "background:#FFF; padding:10px; border-radius:8px; border:1px solid rgba(15,23,42,0.1); cursor:grab; font-size:9.5pt; font-weight:700; transition:all 0.3s; text-align:left; user-select:none;";
    ...
    cardEl.onclick = (e) => {
      ...
      if (selectedCard === cardEl) {
        ...
        cardEl.style.border = "2px solid var(--cyan)";
      }
    };
...
function executeMove(cardId, isDroppedOnSafeLane) {
  const cardEl = document.getElementById(cardId);
  if (!cardEl) return;
  
  const targetLane = isDroppedOnSafeLane ? 
    document.getElementById('lane-safe-dropzone') : 
    document.getElementById('lane-danger-dropzone');
...
function checkSortingAnswers() {
  ...
  let totalCardsCount = sortingCards.length;
  let correctCount = 0;
  let incorrectCount = 0;
  
  const safeCards = safeDz.querySelectorAll('.sorting-draggable-card');
  safeCards.forEach(cardEl => {
    const cardData = sortingCards.find(c => c.id === cardEl.id);
    if (cardData && cardData.type === 'safe') {
  ...
  const dangerCards = dangerDz.querySelectorAll('.sorting-draggable-card');
  dangerCards.forEach(cardEl => {
    const cardData = sortingCards.find(c => c.id === cardEl.id);
    if (cardData && cardData.type === 'danger') {
      cardEl.style.borderColor = "#10B981";
      cardEl.style.boxShadow = "0 0 10px rgba(16,185,129,0.2)";
      cardEl.draggable = false;
      cardEl.style.cursor = "default";
  ...
  if (incorrectCount > 0) {
    sound.playWarning();
  } else if (correctCount === totalCardsCount) {
    sound.playSuccess();
